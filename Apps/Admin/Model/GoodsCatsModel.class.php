...
class GoodsCatsModel extends BaseModel {
    /**
	  ...
	  */
	  public function queryByList($pid = 0){
	     if(I('f')=='api'){
             $rs = $this->where('catFlag=1 and parentId='.(int)$pid)->field('catId,catName')->select();
         }else{
    ...
    public function _getChild($data, $pid, $isClear=false)
    {
	  ...
	  * $cid 分类id
	  */
	 public function getGoodsOrCatByCid($cid){
        $goods = M('goods')->where('goodsCatId3='.(int)$cid)->select();
        $cats = M('goods_cats')->where('parentId='.(int)$cid)->select();
        if($goods or $cats){
            $res = 1;
        }else{
            $res = 0;
