...
class ShopsModel extends BaseModel {
    ...
    /**
	 ...
	 public function insert(){
	 	...
	 	//用户资料
	 	$data = array();
	 	$data["loginName"] = $login = I("loginName");
		$data["loginSecret"] = rand(1000,9999);

		$data["loginPwd"] = md5(I('loginPwd').$data['loginSecret']);
		$data["userName"] = I("userName");
		$data["userPhone"] = $mobile = I("userPhone");
		//店铺资料
		$sdata = array();
...
//        $data["userPhone"] = $mobile = I("userPhone");
        ...
        if($this->checkEmpty($data,true) && $this->checkEmpty($sdata,true)){
            ...
            $data["userType"] = (int)I("userType",1);
            $data["userEmail"] = I("userEmail");
            $data["userQQ"] = I("userQQ",0);
            $data["userScore"] = I("userScore",0);
            $data["userTotalScore"] = I("userTotalScore",0);
            $data["userFlag"] = 1;
            $data["createTime"] = date('Y-m-d H:i:s');
            $m = M('users');
            $userId = $m->add($data); // 创建用户账户
            */
//            if(false !== $userId){
            if(1){
                $sdata["userId"] = 0;
                ...
                $sdata["shopStatus"] = (int)I("shopStatus",1);
                $sdata["shopAtive"] = (int)I("shopAtive",1);
                $sdata["isDistributAll"] = (int)I("isDistributAll",1);
                $sdata["shopFlag"] = 1;
                $sdata["createTime"] = date('Y-m-d H:i:s');
                $sdata['statusRemarks'] = I('statusRemarks');
                $sdata['qqNo'] = I('qqNo',0);
                $sdata["invoiceRemarks"] = I("invoiceRemarks");
                $sdata["shopDesc"] = I("shopDesc");
                $m = M('shops');
                $shopId = $m->add($sdata);
                if(false !== $shopId){
                    $rd = array('code'=>1,'shopid'=>$shopId,'msg'=>'商铺添加成功，请继续添加该商铺管理员账号！');
                    //增加商家评分记录
                    $data = array();
                    $data['shopId'] = $shopId;
                    $m = M('shop_scores');
...
//		$data["shopAddress"] = I("shopAddress");
		...
		$data["bankId"] = I("bankId",0);
		$data["bankNo"] = I("bankNo",0);
		$data["bankUserName"] = I("shopName");
		...
		$data["serviceStartTime"] = I("serviceStartTime",0);
		$data["serviceEndTime"] = I("serviceEndTime",0);
		$data["shopStatus"] = (int)I("shopStatus",0);
		$data["shopAtive"] = (int)I("shopAtive",1);
		$isDistributAll = (int)I("isDistributAll",1);
		$data["isDistributAll"] = $isDistributAll;
		$data["shopTel"] = I("shopTel",0);
     ...
     public function queryByPage(){
        $areaId1 = (int)I('areaId1',0);
	 	...
	 	$sql = "select shopId,shopSn,shopName,u.userName,shopAtive,shopStatus,gc.catName,s.isSelf,s.createTime from __PREFIX__shops s,__PREFIX__users u ,__PREFIX__goods_cats gc 
     ...
     public function queryPeddingByPage(){
        $areaId1 = (int)I('areaId1',0);
     	$areaId2 = (int)I('areaId2',0);
	 	$sql = "select shopId,shopSn,shopName,u.userName,shopAtive,shopStatus,gc.catName,s.createTime from __PREFIX__shops s,__PREFIX__users u ,__PREFIX__goods_cats gc 
