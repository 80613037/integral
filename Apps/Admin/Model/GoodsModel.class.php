	 ...
	 */
     ...
     public function queryPenddingByPage(){
        $shopName = WSTAddslashes(I('shopName'));
         ...
         if(I('channel')>0){
             if(isset($channel))$sql .=" and g.channel= ".$channel ;
         }
     ...
     public function queryByPage(){
        $score = (int)I('score',0); // 积分商城商品
        $miaos = (int)I('miaos',0); // 秒杀商品
        $pintuan = (int)I('pintuan',0); // 拼团商品
        $shopName = WSTAddslashes(I('shopName'));
     	$goodsName = WSTAddslashes(I('goodsName'));
     	$areaId1 = (int)I('areaId1',0);
     	$areaId2 = (int)I('areaId2',0);
	 	...
	 	if($shopName!='')$sql.=" and (p.shopName like '%".$shopName."%' or p.shopSn like '%".$shopName."%')";
	 	if($goodsName!='')$sql.=" and (g.goodsName like '%".$goodsName."%' or g.goodsSn like '%".$goodsName."%')";
     // 积分商城商品 和  秒杀商品  // ADD BY Yang
        if(isset($score) && $score==1)$sql .=" and g.isScore=1 ";
        if(isset($miaos) && $miaos==1)$sql .=" and g.isMiaosha=1 ";
        if(isset($pintuan) && $pintuan==1)$sql .=" and g.ispintuan=1 ";
        if(I('channel')>0){
            if(isset($channel))$sql .=" and g.channel= ".$channel ;
        }
        if(I('sort')=='sc'){
	 ...
	 public function changeGoodsStatus(){
	 	...
	 	foreach($ids as $k=>$id)
	 	{
            ...
            if(C('isDevelop')){
                WLog('log','sql144_', $rs.$this->getLastSql());
            }
            $rd['code'] = $rs;
			if(false !== $rs){
