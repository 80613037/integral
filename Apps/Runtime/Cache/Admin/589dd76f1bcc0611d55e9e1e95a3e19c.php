...
dl.areaSelect dd{ float: left; margin-left: 20px; cursor: pointer;}
   ...
   $(function () {
	   ...
	   $("#loginPwd").formValidator({
		...
		$("#shopSn").formValidator({onShow:"",onFocus:"商户编号不能超过20个字符",onCorrect:"输入正确"}).inputValidator({min:1,max:20,onError:"商户编号不符合要求,请确认"});
		$("#shopName").formValidator({onShow:"",onFocus:"商户名称不能超过20个字符",onCorrect:"输入正确"}).inputValidator({min:1,max:40,onError:"商户名称长度不符合要求,请确认"}).ajaxValidator({
			dataType : "json",
			async : true,
			url :  "<?php echo U('Admin/Shops/checkShopName',array('id'=>$object['shopId']));?>",
			success : function(data){
				var json = WST.toJson(data);
	            if( json.status == "1" ) {
	                return true;
		...
		$("#shopAddress").formValidator({onShow:"",onFocus:"请输入商户地址",onCorrect:"输入正确"}).inputValidator({min:1,max:120,onError:"商户地址不能为空,请确认"});
...
//		initTime('serviceEndTime','<?php echo ($object['serviceEndTime']); ?>');
       initTime('serviceStartTime','0');
       initTime('serviceEndTime','0');
		getCommunitys();
   ...
   function edit(){
	   ...
	   if(params.shopImg=='' && params.shopStatus!=-1 ){
		   Plugins.Tips({title:'信息提示',icon:'error',content:'请上传商户图片!',timeout:1000});
		   return;
	   }
	   ...
	   if(params.shopStatus==1){
		   if(params.latitude=='' || params.longitude==''){
			   Plugins.Tips({title:'信息提示',icon:'error',content:'请在地图上标注商户所在位置!',timeout:1000});
			   return;
		   }
	   }
	   ...
	   if(params.relateAreaId=='0' && params.relateCommunityId=='0' && params.shopStatus!=-1 && params.isDistributAll!=1){
       ...
       <form name="myform" method="post" id="myform" autocomplete="off">
        ...
        <table class="table table-hover table-striped table-bordered wst-form">
           ...
           <tr>
             ...
             <td><input type='password' id='loginPwd' class="form-control wst-ipt" value='<?php echo ($object["loginPwd"]); ?>' maxLength='20'/></td>
           </tr><?php endif; ?>
           <tr>
             <th align='right'>商户编号<font color='red'>*</font>：</th>
             <td><input type='text' id='shopSn' class="form-control wst-ipt" value='<?php echo ($object["shopSn"]); ?>' maxLength='25'/></td>
           </tr>
           <tr>
             <th align='right'>商户名称<font color='red'>*</font>：</th>
             <td><input type='text' id='shopName' name="shopName" class="form-control wst-ipt" value='<?php echo ($object["shopName"]); ?>' maxLength='25'/></td>
           </tr>
           <tr>
           ...
           <tr style="height:80px;">
             ...
             <td>
             	<div>
             	    ...
             	    <input id="shopImg" name="shopImg" class="text wstipt" tabindex="3" autocomplete="off" type="hidden" value="<?php echo ($object["shopImg"]); ?>"/>
             		<div style="clear:both;"></div>
             	</div>
	             <div id="preview_Filedata">
	             	<img id='preview' src='/<?php echo ($object["shopImg"]); ?>' ref='/<?php echo ($object["shopImg"]); ?>' <?php if($object['shopImg'] =='' ): ?>style='display:none'<?php else: ?>style='width:152px;height:152px;max-width:152px;'<?php endif; ?>/>
	             </div>
             </td>
           </tr>
           <tr>
           ...
           <tr>
             ...
             <td><input type='text' id='bankUserName' class="form-control wst-ipt" value='<?php echo ($object["bankUserName"]); ?>' maxLength='25'/></td>
           </tr>
           <tr>
           ...
           <tr style="display: none;">
            ...
            <tr>
             <th align='right'>商户状态<font color='red'>*</font>：</th>
             <td>
             <label>
