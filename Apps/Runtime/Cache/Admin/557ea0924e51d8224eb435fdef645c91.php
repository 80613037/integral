...
<html lang="zh-cn">
   ...
   function toggleIsShow(t,v){
	   ...
	   $.post("<?php echo U('Admin/transport/editiIsShow');?>",{id:v,isShow:t},function(data,textStatus){
   ...
   function del(id){
	   Plugins.confirm({title:'信息提示',content:'您确定要删除该文章吗?',okText:'确定',cancelText:'取消',okFun:function(){
		   Plugins.closeWindow();
		   Plugins.waitTips({title:'信息提示',content:'正在操作，请稍后...'});
		   $.post("<?php echo U('Admin/transport/del');?>",{id:id},function(data,textStatus){
					var json = WST.toJson(data);
					if(json.status=='1'){
						Plugins.setWaitTipsMsg({content:'操作成功',timeout:1000,callback:function(){
						location.reload();
   ...
   <body class='wst-page'>
       <form method='post' action='<?php echo U("Admin/transport/index");?>'>
           <div class='wst-tbar' style='height:25px;'>

               <?php if(in_array('kdgl_01',$WST_STAFF['grant'])){ ?>
               <a class="btn btn-success glyphicon glyphicon-plus" href="<?php echo U('Admin/transport/toEdit');?>" style='float:right'>新增</a>
               <?php } ?>
           </div>
       </form>
       <div class="wst-body"> 
        <table class="table table-hover table-striped table-bordered wst-list">
           ...
           <tbody>
            <?php if(is_array($Page['root'])): $i = 0; $__LIST__ = $Page['root'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
               ...
               <?php if(in_array('kdgl_02',$WST_STAFF['grant'])){ ?>
               ...
               <td>
               <?php if(in_array('kdgl_02',$WST_STAFF['grant'])){ ?>
               <a class="btn btn-default glyphicon glyphicon-pencil" href="<?php echo U('Admin/transport/toEdit',array('id'=>$vo['id']));?>">修改</a>&nbsp;
               <?php } ?>
               <?php if(in_array('kdgl_03',$WST_STAFF['grant'])){ ?>
