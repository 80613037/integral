...
class ShopsModel extends BaseModel {
    ...
    */
	...
	/**
	 ...
	 public function addByVisitor(){
		...
		$shopRules = array(
		     ...
		     array('shopName','require','请输入商户名称!',1),
		     ...
		     array('latitude','require','请标记商户地址!',1),
		     array('longitude','require','请标记商户地址!',1),
		     array('mapLevel','integer','请标记商户地址!',1),
		     array('isInvoice',array(0,1),'无效的开发票状态！',1,'in'),
		     array('serviceStartTime','double','请选择商户开始时间!',1),
		     array('serviceEndTime','double','请选择商户结束时间!',1)
		);
		$duser = D('Home/Users');
	 	//检测账号是否存在
	    ...
	    if($hasLoginName['status']!=1){
	 	...
	 	if($hasShopName['status']!=1){
	 		$rd = array('status'=>-8,'msg'=>($hasShopName['status']==-2)?"不能使用该商户名称":"该商户名称已存在");;
	 		return $rd;
	 	}
	 	$u = M('users');
	 	$s = M('shops');
        ...
        $mobile = I("userPhone");
	    // 同步注册 众筹用户
         $sqlzc = "INSERT INTO fanwe_user(user_name,user_pwd,mobile,is_effect,create_time,update_time,user_level,user_type) VALUES('$login', '$pwd', '$mobile', 1, $ct, $ct, 8, 11)";
         $ret = M()->query($sqlzc);
         if(C('isDevelop')){
             WLog('log','L152sql', $sqlzc);
             WLog('log','L153return', $ret);
         }
	 ...
	 public function addByUser($userId){
		...
		if($checkRs['counts']>0){
	 	...
	 	$shopRules = array(
		     ...
		     array('mapLevel','integer','请标记商户地址!',1),
		     array('isInvoice',array(0,1),'无效的开发票状态！',1,'in'),
		     array('serviceStartTime','double','请选择商户开始时间!'),
	 ...
	 public function edit($shopId,$isApply=false){
	    ...
	    $hasShopName = $this->checkShopName(I('shopName'),$shopId);
	 	if($hasShopName['status']==0){
	 		$rd = array('status'=>-8,'msg'=>'该商户名称已存在!');;
	 		return $rd;
	 	}
     ...
     * @return mixed
     */
	public function getMallInfo($fieldCode, $field){
        $res = M('sys_configs')->where(array('fieldCode'=>$fieldCode))->getField($field);
        return $res;
    }
