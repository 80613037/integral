...
class GoodsModel extends BaseModel {
	...
	public function getGoodsList($obj){
        ...
        if(C('isDevelop')){
            WLog('log','L95sql', $sqla . $sql . $where);
        }
	 ...
	 * 商城首页
	 */
    public function getGoodsList_mall($status, $limit){
        $goods = M('goods')->where(array('isAdminRecom'=>$status,'isSale'=>1,'goodsStatus'=>1,'goodsFlag'=>1,'isSaleout'=>0,'isMiaosha'=>0,'ispintuan'=>0,'isScore'=>0))->order('goodsId desc')->limit($limit)->select();
        return $goods;
    }

    /**
     *
     * @param $status  商城推荐1   -1则去掉该条件
     * @param $type    商品类型
     * @param $limit
     * @param int $mstime
     * @return mixed
     */
    public function getGoodsList_index($status, $type, $limit, $mstime=0){
        //$goods = M('goods')->where(array('isAdminRecom'=>$status,'isSale'=>1,'goodsStatus'=>1,'goodsFlag'=>1,'isSaleout'=>0,'isMiaosha'=>0,'ispintuan'=>0,'isScore'=>0))->order('goodsId desc')->limit(2)->select();
        $sql = 'SELECT * FROM `__PREFIX__goods` WHERE `isSale` = 1 AND `goodsStatus` = 1 AND `goodsFlag` = 1 AND `isSaleout` = 0  ';

        if($status != -1){
            $sql .= ' AND `isAdminRecom` = '.$status ;
        }
        if($type == 'miaosha'){
            $sql .= ' AND `isMiaosha` = 1 AND `ispintuan` = 0 AND `isScore` = 0 ';
        }elseif($type == 'pintuan'){
            $dateToday = date('Y-m-d H:i:s');
            $sql .= " AND `isMiaosha` = 0 AND `ispintuan` = 1 AND `isScore` = 0 AND ptlastdate > '$dateToday'";
        }elseif($type == 'score'){
            $sql .= ' AND `isMiaosha` = 0 AND `ispintuan` = 0 AND `isScore` = 1 ';
        }else{
            $sql .= ' AND `isMiaosha` = 0 AND `ispintuan` = 0 AND `isScore` = 0 ';
        }
        if($mstime != 0){
            $sql .= ' AND `ismiaoshatime` ='.$mstime;
        }
        $sql .= '  ORDER BY goodsId desc LIMIT '.$limit;
        if(C('isDevelop')){
            WLog('155','date',$sql);
        }
    ...
    public function getMsGoodsIndex(){
        $sql = 'SELECT * FROM `__PREFIX__goods` WHERE `isAdminRecom`=1 AND `isSale` = 1 AND `goodsStatus` = 1 AND `goodsFlag` = 1 AND `isSaleout` = 0  ';
        $sql .= ' AND `isMiaosha` = 1 AND `ispintuan` = 0 AND `isScore` = 0 ';
        $dateH = date("H");
        if(($dateH>=12) && ($dateH<16) ){
            $sql .= ' AND `ismiaoshatime` = 12';
        }elseif(($dateH>=16) && ($dateH<20) ){
            $sql .= ' AND `ismiaoshatime` = 16';
        }elseif(($dateH>=20) && ($dateH<22) ){
            $sql .= ' AND `ismiaoshatime` = 20';
        }
        $sql .= '  ORDER BY goodsId desc LIMIT 2';
	...
	public function getGoodsDetails($obj){		
		...
		$sql = "SELECT sc.catName,sc2.catName as pCatName, g.*,shop.shopName,shop.deliveryType,ga.id goodsAttrId,ga.attrPrice,ga.attrStock,
				...
				shop.deliveryMoney, shop.isSelf ,g.goodsSn,shop.serviceStartTime,shop.serviceEndTime FROM __PREFIX__goods g left join __PREFIX__goods_attributes ga on g.goodsId=ga.goodsId and ga.isRecomm=1, __PREFIX__shops shop, __PREFIX__shops_cats sc 
	...
	public function getPingjia($gid){
        $sql = "select u.userName,u.userPhoto,pj.content,pj.appraisesAnnex,pj.createTime from __PREFIX__users as u left join __PREFIX__goods_appraises as pj on u.userId=pj.userId where pj.goodsId=" . $gid . " and pj.isShow=1 order by pj.id desc";
	...
	public function queryOnSaleByPage(){
        ...
        $score = (int)I('score',0);
        $miaos = (int)I('miaos',0);
        $pintuan = (int)I('pintuan',0);
		$goodsName = WSTAddslashes(I('goodsName'));
		$sql = "select g.goodsId,g.goodsSn,g.goodsName,g.goodsImg,g.goodsThums,g.shopPrice,g.goodsStock,g.saleCount,g.isSale,g.isSaleout,g.isRecomm,g.isHot,g.isBest,g.isMiaosha,g.isScore,g.ispintuan,g.exchangeScore,g.ismiaoshatime,g.ptrs,g.ptlastdate,g.isNew,g
		.saleTime,ga.isRecomm as attIsRecomm from __PREFIX__goods g
				left join __PREFIX__goods_attributes ga on g.goodsId = ga.goodsId and ga.isRecomm = 1
				where g.goodsFlag=1 
        ...
        if($score == 1){$sql .= " AND g.isScore=1 ";}
        if($miaos == 1){$sql .= " AND g.isMiaosha=1 ";}
        if($pintuan == 1){$sql .= " AND g.ispintuan=1 ";}
        if(I('sort')=='sc'){
            $sql.=" order by saleCount desc limit ".I('limit',3);
            return $this->query($sql);
        }else{
	...
	public function queryUnSaleByPage(){
		...
		$shopCatId2 = (int)I('shopCatId2',0);
		$goodsName = WSTAddslashes(I('goodsName'));
		$sql = "select g.goodsId,g.goodsSn,g.goodsName,g.goodsImg,g.goodsThums,g.shopPrice,g.goodsStock,g.saleCount,g.isSale,g.isRecomm,g.isHot,g.isBest,g.isNew,g.isMiaosha,g.isScore,ga.isRecomm as attIsRecomm from __PREFIX__goods  g
				left join __PREFIX__goods_attributes ga on g.goodsId = ga.goodsId and ga.isRecomm = 1
				where g.goodsFlag=1 
	...
	public function queryPenddingByPage(){
		...
		$shopCatId2 = (int)I('shopCatId2',0);
		$goodsName = WSTAddslashes(I('goodsName'));
		$sql = "select g.goodsId,g.goodsSn,g.goodsName,g.goodsImg,g.goodsThums,g.shopPrice,g.goodsStock,g.saleCount,g.isSale,g.isRecomm,g.isHot,g.isBest,g.isMiaosha,g.isScore,g.isNew,g.ispintuan,g.ptrs,g.ptlastdate,ga.isRecomm as attIsRecomm from __PREFIX__goods g
				left join __PREFIX__goods_attributes ga on g.goodsId = ga.goodsId and ga.isRecomm = 1
				where g.goodsFlag=1 
	...
	protected $_validate = array(
		 ...
		 array('goodsImg','require','请上传商品图片0!',1),
...
//		 array('goodsUnit','require','请输入商品单位!',1),
    ...
    public function insert(){
        $rd = array('status'=>-1);
        //查询商家状态
        $shopId = (int)I('shopid', 36); // (int)session('WST_USER.shopId');
        $sql = "select shopStatus from __PREFIX__shops where shopFlag = 1 and shopId=".$shopId;
        $shopStatus = $this->query($sql);
        if(empty($shopStatus)){
            $rd['status'] = -2;
        ...
        if (1){
            ...
            $m->shopPrice = I('shopPrice');
            $m->marketPrice = I('marketPrice');
            $m->goodsCatId1 = I('goodsCatId1');
            $m->goodsCatId2 = I('goodsCatId2');
            $m->goodsCatId3 = I('goodsCatId3');
            $m->shopCatId1 = I('shopCatId1');
            $m->shopCatId2 = I('shopCatId2');
            $m->goodsStock = (int)I('goodsStock');
            $m->exchangeScore = I('exchangeScore'); // 注：前提需 isScore=1（即I("isActive")=2）
            $m->rebateScore = I('rebateScore');  // 注: 需要 isRebate = 1

            $m->isBest = ((int)I('isBest')==1)?1:0;
            $m->isRecomm = ((int)I('isRecomm')==1)?1:0; // 推荐到店铺首页
            $m->isNew = ((int)I('isNew')==1)?1:0;
            $m->isHot = ((int)I('isHot')==1)?1:0;
            //如果商家状态不是已审核则所有商品只能在仓库中
            if($shopStatus[0]['shopStatus']==1){
                $m->isSale = 1; // ((int)I('isSale')==1)?1:0;
            }else{
                $m->isSale = 0;
            }
            if($m->isSale==1)$m->saleTime=date('Y-m-d H:i:s');
	...
	public function getShopsGoods($shopId = 0){
		...
		$sql = "SELECT sp.shopName, g.saleCount totalnum, sp.shopId ,g.goodsStock, g.goodsId , g.goodsName,g.goodsImg, g.goodsThums,g.shopPrice,g.marketPrice, g.goodsSn,g.ispintuan,g.isMiaosha,g.isScore,ga.id goodsAttrId 
        ...
        if($type == 'ms'){
            $sql .= " AND g.isMiaosha = 1 ";
        }elseif($type == 'pt'){
            $sql .= " AND g.ispintuan = 1  AND g.ptlastdate > '$dateToday'";
        }elseif($type == 'zc'){
            $sql .= " AND g.iszhongchou = 1 ";
        }
        if(C('isDevelop')){
            WLog('1006','date',$sql);
        }
...
//	 		  	    and g.goodsId = ".$goodsId;
//                $goodsRs = $this->queryRow($sql);
//                if(empty($goodsRs))return array('status'=>-1,'msg'=>'失败!');
////                $data["saleTime"] = date('Y-m-d H:i:s');
//            }
        }
		$s = M('goods')->where("goodsId=$goodsId")->save($data);
if($s){
    $rdata= array("status"=>1,'msg'=>'操作成功!');
}
//		$rdata["status"] = 1;
		return $rdata;
	}
	...
	public function getPintuanInfo($goodsid){
		// buyerid=leaderid 则该用户就是团长；取最新的五条
		$sql = "SELECT distinct op.leaderid,u.userName,u.userPhoto,op.enddate FROM __PREFIX__orders_pintuan op LEFT JOIN __PREFIX__users u ON u.userId=op.leaderid WHERE goodsid=".$goodsid." AND op.pintuanStatus=1 ORDER BY op.orderId DESC LIMIT 0,5";
		$buyerInfos = $this->query($sql);
        foreach($buyerInfos as $key=>$val){
            $count = M('orders_pintuan')->where(array('goodsid'=>$goodsid,'leaderid'=>$val['leaderid'],'pintuanStatus'=>1))->count('orderId');
            $buyerInfos[$key]['count'] = $count;
        }
		$res['tuanzhang'] = $buyerInfos;
		return $res;
    }

    public function isSelf(){
        $shopId=(int)session('WST_USER.shopId');
        $isSelf = M('shops')->field('isSelf')->where('shopId='.$shopId)->find();
        return $isSelf;
    }
     ...
     * 众筹商品
     */
    public function getGoodsList_zc($limit){
        $now = time();
        $sql = "SELECT id,name,image,limit_price,support_amount FROM fanwe_deal WHERE is_recommend=1 AND type=0 AND is_delete=0 AND success_time=0 AND end_time>".$now." ORDER BY id DESC LIMIT $limit";
        return $this->query($sql);

    }
/**
    public function get_deal_list($limit="",$conditions="",$orderby=" sort asc "){
        ...
        $deal_count = $GLOBALS['db']->getOne("select count(*)  from fanwe_deal as d  where ".$condition);
