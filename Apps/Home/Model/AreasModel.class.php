	...
	/**
	  ...
	  */
	  ...
	  public function getDefaultCity(){
          ...
          if(C('isDevelop')){ WLog('log','L111', $areaId2);}
	  	if($areaId2==0){
	  		$areaId2 = (int)session('areaId2');
	  	}
          if(C('isDevelop')){ WLog('log','L115', $areaId2);}
	  	//检验城市有效性
	  	if($areaId2>0){
          ...
          if(C('isDevelop')){ WLog('log','L124', (int)$_COOKIE['areaId2']);}
	  	//定位城市
	  	if($areaId2==0){
	  		//IP定位
	  		$iparea = WSTIPAddress();
            if(C('isDevelop')){ WLog('log','L129', $iparea);}
	  		if(!empty($iparea)){
	  			$where = array();
