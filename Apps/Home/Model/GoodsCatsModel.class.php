	...
	*/
	...
	public function getGoodsCatsAndGoodsForIndex($areaId2){
		...
		if(!$cats){
			...
			$sql = "select catId,catName,parentId from __PREFIX__goods_cats WHERE parentId = 0 AND isShow =1 AND isFloor = 1 AND catFlag = 1 order by catSort asc limit 10";
			$rs1 = $this->query($sql);
			$cats = array();
			//取出所有一级分类下对应的被推荐的二级分类,上限8个
			for ($i = 0; $i < count($rs1); $i++) {
				$cat1Id = $rs1[$i]["catId"];
				$sql = "select catId,catName,parentId from __PREFIX__goods_cats WHERE parentId = $cat1Id AND isShow =1 AND isFloor = 1 AND catFlag = 1 order by catSort asc limit 8";
				$rs2 = $this->query($sql);
				$cats2 = array();
				for ($j = 0; $j < count($rs2); $j++) {
					//取出二级分类下对应的被推荐的三级分类的前10个,可通过修改排序来调整顺序
					$cat2Id = $rs2[$j]["catId"];
					$sql = "select catId,catName,parentId from __PREFIX__goods_cats WHERE parentId = $cat2Id AND isShow =1  AND isFloor = 1 AND catFlag = 1 order by catSort asc";
					$rs3 = $this->query($sql);
					$cats3 = array();
