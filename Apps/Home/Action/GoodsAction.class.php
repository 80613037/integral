...
class GoodsAction extends BaseAction {
    ...
    public function getGoodsList(){
        ...
        $this->assign('catList',$catList['0']['catChildren']);
   		$this->display('mobile/goods_list');
    }
	...
	public function getGoodsDetails(){
		...
		if($kcode==$scrictCode || ($goodsDetails["isSale"]==1 && $goodsDetails["goodsStatus"]==1)){
            ...
            // 评价
            $pingjia = $goods->getPingjia($goodsId);
            foreach ($pingjia as $key=>$v){
                $pingjia[$key]['img'] = explode(',', $v['appraisesAnnex']);
            }
            $this->assign('userid',$userId);
            $this->assign('pingjia',$pingjia);
			$this->display('mobile/goods_details');
		}else{
            $this->assign('notexist',1);
            $this->display('mobile/goods_details');
//			$this->display('mobile/goods_notexist');
		}
	}
	...
	*/
	public function queryOnSaleByPage(){
        ...
        if(I("f")=='api'){
            if(I('sort')=='sc'){
                $res['data']=$m->queryOnSaleByPage($USER['shopId']);
            }else{
                $res['data'] = $page['root'];
	...
	*/
     ...
     * 拼团商品详情
     */
    public function getGoodsDetailsPt(){
//        $this->isUserLogin();
        ...
        if($kcode==$scrictCode || ($goodsDetails["isSale"]==1 && $goodsDetails["goodsStatus"]==1)){
            ...
            // 判断是否已经是 团长
            $uid = session('WST_USER.userId');
            $isTuanzhang = M('orders_pintuan')->where(array('leaderid'=>$uid,'buyerid'=>$uid,'goodsid'=>$goodsId))->find();
            if(!empty($isTuanzhang)){
                $state = 1;
            }else{
                $state = 0;
            }
            $this->assign('tuanzhang', $state);

            $isLate = ((time()-strtotime($goodsDetails['ptlastdate']))<0) ? 0 : 1;
            $this->assign('isLate', $isLate);

            // 拼团信息
            $pintuanInfo = $goods->getPintuanInfo($goodsId);
            $this->assign('uInfo', $pintuanInfo['tuanzhang']);
            $this->assign('uid', $uid);
            $this->display('mobile/goods_details_pt');
        }else{
            $this->display('mobile/goods_notexist');
        }
    }
     ...
     *
     */
    public function getGoodsDetailsScore(){
        $goods = D('Home/Goods');
        $kcode = I("kcode");
        $scrictCode = md5(base64_encode("wstmall".date("Y-m-d")));
        //查询商品详情
        ...
        if($kcode==$scrictCode || ($goodsDetails["isSale"]==1 && $goodsDetails["goodsStatus"]==1)){
            ...
            $pintuanInfo = $goods->getPintuanInfo($goodsId);
            $this->assign('uInfo', $pintuanInfo['tuanzhang']);
            $this->display('mobile/goods_details_jf');
        }else{
            $this->display('mobile/goods_notexist');
        }
    }
