...
function saleout(v){
	...
	layer.confirm((v==1)?"您确定要将这些商品设置售罄？":"您确定要将这些商品取消售罄？",{icon: 3, title:'系统提示'},function(tips){
		var loading = layer.load('正在处理，请稍后...', 3);
		layer.close(tips);
		var params = {};
		params.ids = ids;
		params.isSaleout= v;
		$.post(Think.U('Home/Goods/saleout'),params,function(data,textStatus){
			layer.close(loading);
			var json = WST.toJson(data);
			if(json.status=='1'){
				WST.msg('操作成功！', {icon: 1},function(){
				...
				WST.msg('00已成功上架商品'+json.num+" 件，请核对未能上架的商品信息是否完整。", {icon: 5},function(){
					location.reload();
				});
			}else if(json.status=='-3'){
				WST.msg('设置售罄失败!您的店铺权限不能出售商品，如有疑问请与商城管理员联系。', {icon: 5,time:3000});
			}else{
				WST.msg('操作失败!', {icon: 5});
			}
		});
...
function showOrder(id){
	layer.open({
	    ...
	    area: ['1024px', ($(window).height() - 100) +'px']
	});
}
//受理
...
//发货配送
function shopOrderDelivery(id,uid){
	window.location.href="/index.php?m=Home&c=orders&a=toWriteTransport&orderId="+id+"&userId="+uid;
...
function queryOrderPager(statusMark,pcurr){
	...
	$.post(Think.U('Home/Orders/queryShopOrders'),param,function(data,textStatus){
			...
			if(json.root.length>0){
				...
				if(order.isScore == 1){
					html.push("<td width='100' style='color:indianred;'>"+order.exchangeScore+"积分</td>");
					html.push("<td width='100' style='font-weight:bold; color:indianred;'>"+order.exchangeScore+"积分</td>");
				}else{
				...
				if(order.ispintuan == 1){
					html.push("<td width='100' style='color:red;'>拼团商品订单</td>");
				}else if(order.isMiaosha == 1) {
                    html.push("<td width='100' style='color:red;'>秒杀商品订单</td>");
                }else if(order.isScore == 1) {
					html.push("<td width='100' style='color:indianred;'>积分商城订单</td>");
				}else{
					html.push("<td width='100'>普通订单</td>");
				}
					...
					if(order.orderStatus==0){
						...
						html.push(" | <a href='javascript:;' style='color:"+((order.orderStatus==-6 || order.orderStatus==-3)?"red":"blue")+"' onclick=shopOrderDelivery('"+order.orderId+"','"+order.userId+"')>发货配送</a>");
...
function editShop(){
	   ...
	   params.shopDesc = ($('#shopDesc').val()=='') ? $('#shopName').val() : $('#shopDesc').val();
