...
function addAccess(aid){
	...
	$.post('index.php?m=Home&c=Index&a=access',{id:aid},function(data,textStatus){
	});
}
...
function checkCartPay(shopId,goodsId,num,ischk,isBook,goodsAttrId){
	...
	jQuery.post( '/index.php?m=Home&c=Cart&a=changeCartGoodsNum' ,{goodsId:goodsId,num:num,ischk:ischk,goodsAttrId:goodsAttrId,isBook:isBook},function(data) {
		// var json = WST.toJson(data);
		if(data.goodsStock==0){
			$("#stock_"+data.goodsId).html("<span style='color:red;'>无货</span>");
		}
		num = parseInt(num,10);
		if(data.goodsStock>=num){
			num = num>100?100:num;
			$("#stock_"+data.goodsId+"_"+goodsAttrId).html("有货");
			$("#selgoods_"+data.goodsId+"_"+goodsAttrId).css({"border":"0"});
		}else{
			num = json.goodsStock;
			$("#stock_"+data.goodsId+"_"+goodsAttrId).html("<span style='color:red;'>仅剩最后"+data.goodsStock+"份</span>");
			$("#selgoods_"+data.goodsId+"_"+goodsAttrId).css({"border":"0"});
		}
