...
function queryGoods(obj,mark){
	...
	msort = $('#msort').val();
	msort = (msort=='0')?1:0;
	window.location = window.location.href+'&msort='+msort+'&mark='+mark;
}
...
function addCart(goodsId,type,goodsThums){
	...
	if($("#userid").val()==0){
        layer.msg('您还未登录，请先登录!', function(){
            location.href= '/index.php?m=Home&c=Users&a=login';
        });
	}
	var params = {};
	...
	jQuery.post('/index.php?m=Home&c=Cart&a=addToCartAjax',params,function(data) {
		// var json = WST.toJson(data);
		if(data.status==1){
			if(type==1){
				location.href='/index.php?m=Home&c=Cart&a=toCart';
			}
		}else{
			// WST.msg(data.msg,{offset: '200px'});
		}
	});
}
