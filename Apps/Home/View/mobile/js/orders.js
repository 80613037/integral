...
function submitOrder_new(payway){
    ...
    // var ll = layer.msg('正在提交订单，请稍候...', {icon: 16,shade: [0.5, '#B3B3B3']});
    jQuery.post("/index.php?m=Home&c=Goods&a=checkGoodsStock", {}, function(data) {
        var goodsInfo = data;
        // layer.close(ll);
        for(var i=0;i<goodsInfo.length;i++){
            var goods = goodsInfo[i];
            if(goods.isSale<1){
                ...
                WST.msg('商户'+goods.shopName+'在休息中，不能下单!', {icon: 5});
                return;
            }
        }

        var params = {};
        params.coupid = $('#coupid').val();
        ...
        jQuery.post(Think.U('Home/Orders/submitOrder') ,params,function(data) {
            ...
            if(json.status==1){
				...
				if(json.status == 33){
					WST.msg(json.msg, {icon: 5});
					setTimeout('window.location.href="/index.php?m=Home&c=Users&a=index";',3000);
				}
            }
        });
    });
		...
		jQuery.post(Think.U('Home/Orders/payYue') ,{'oids':oids, 'totalMoney':$('#totalMoney').val(), 'gold':$('#gold').val()},function(data) {
			...
			if(data){
				if(data.code==0){
					...
					jQuery.post(Think.U('Home/Orders/changeOrdertatusAndorderLog') ,{orderids:oids},function(data) {
						if(data==1){
							setTimeout('window.location.href="/index.php?m=Home&c=Users&a=index";',2000);
						}
					})
